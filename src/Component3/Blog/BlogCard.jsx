  ...
  return (
    ...
    >
      ...
      <div className='relative z-20 rounded-lg transition-all duration-500 px-3 pb-3'>
        ...
        <Link to={blogUrl}>
          <button
            className={`font-Outfit text-left font-medium text-lg sm:text-[22px] md:text-xl lg:text-2xl xl:text-[21px] 2xl:text-2xl text-HeadingColor-0 transition-all duration-500 mb-7 ${titleHover}`}
          >
            {blogTitle}
          </button>
        </Link>
        <Link to={blogUrl}>
          <button
            className={`font-Outfit flex items-center justify-between w-full pl-8 pr-6 py-[13px] border relative z-10 overflow-hidden rounded-full transition-all duration-500 group-hover:!text-white 
