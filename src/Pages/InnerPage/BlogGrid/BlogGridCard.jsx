    ...
    <div className='group bg-BodyBg-0 p-5 rounded-md overflow-hidden'>
      <div className='relative overflow-hidden rounded-md'>
        ...
        <div className='absolute bottom-4 -right-1/4 transition-all duration-500 opacity-0 group-hover:opacity-100 group-hover:right-6'>
          <Link to={blogGridUrl}>
            <button className='font-OpenSans text-white size-11 rounded-full border-2 text-2xl flex justify-center items-center border-white bg-PrimaryColor-0'>
              {thumbBtn}
            </button>
          </Link>
        </div>
      ...
      <div className='sm:px-5 lg:px-0 xl:px-5 relative z-10'>
        ...
        <Link to={blogGridUrl}>
          <button className='font-Outfit font-semibold text-left text-lg sm:text-[22px] lg:text-xl xl:text-[22px] 2xl:text-2xl text-HeadingColor-0 transition-all duration-500 hover:text-PrimaryColor-0 mt-7 mb-3'>
            {blogGridTitle}
          </button>
        </Link>
        <p className='font-OpenSans text-TextColor2-0 pb-4'>{blogGridDesc}</p>
      </div>
    </div>
  );
};
